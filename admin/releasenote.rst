...
-  :ref:`api-cmd-purge` 기능 고도화 - 가상 호스트 :ref:`adv-vhost-sub-path` 시 Purge 계열 API가 Sub 가상 호스트를 올바르게 타깃으로 처리

-  :ref:`https-conf` 설정
   
   -  인증서 - 심볼릭 링크 파일 인식 개선
   -  Let’s Encrypt RSA 인증서 지원

-  비정상 적으로 인코딩 된 MP4 파일 호환성 강화
-  디스크 인덱스 파일 병렬 로딩으로 구동 속도 개선


.. _release-cdn-2-12-15:
...
.. _release-cdn-2-12-14:

2.12.14 (2026.05.14)
----------------------------

-  비동기 퍼지 완료 동작에 대한 ``PreCacheControl`` 리스트 개선
-  :ref:`handling_http_requests_modify_client` - 클라이언트 요청 쿼리 스트링 값을 응답 하는 기능


.. _release-cdn-2-12-13:

2.12.13 (2026.04.28)
----------------------------

-  Let's Encrypt ``ECDSA`` 인증서 지원


.. _release-cdn-2-12-12:

2.12.12 (2026.03.04)
----------------------------

-  :ref:`media-hls` - m3u8 상대 경로 지원
-  :ref:`media-mp3-hls` - m3u8 상대 경로 지원
-  :ref:`admin-log-access-custom` - ``%b`` 전송량 음수가 나오는 증상 수정
...
-  비동기 퍼지 요청시 간헐적으로 ``412`` 응답코드가 서비스되던 증상 수정
-  설정 백업시 DIMS 설정이 누락 되는 증상 수정


.. _release-cdn-2-12-10:

2.12.10 (2026.01.21)
----------------------------

-  메모리 모드에서 ``TotalSize`` 통계 버그 수정
-  :ref:`api-monitoring-fileinfo` API - 응답 JSON 문법오류 수정
-  WM - 보안취약점 수정 (KVE-2025-2813, KVE-2025-2814)
...
-  :ref:`admin-log-access` - :ref:`admin-log-accessextra` 기능 추가
-  ``/monitoring/average`` API에서 멀티 가상호스트 지정 가능 추가
-  응답코드 없어 끊어지거나 ``1xx`` 계열의 클라이언트 응답코드 통계가 집계되지 않던 문제 수정
-  설정 백업 - 안정성 개선
...
-  :ref:`https-api` 추가
-  설정 백업 - 파일 순서에 따라서 설정 HASH가 달라지는 증상 개선
...
-  가상호스트 삭제 API 호출시 ``vhosts.xml`` 들여쓰기 수정
...
-  설정 백업 - 파일 변경 시간에 따라서 설정 HASH가 달라지는 증상 개선
-  :ref:`handling_http_requests_modify_client` - 요청 URL을 지칭하는 ``#URI`` 예약어 추가
...
-  완료된 비동기 Purge ID 최대 저장 개수를 설정 하는 기능 추가
-  완료된 비동기 Purge ID 검색 기능 추가
-  삭제된 가상호스트의 설정은 백업 되지 않도록 수정
-  WM - ``<MaxSockets>`` 설정 임계치 변경
...
-  :ref:`access-control-vhost` 의 :ref:`access-control-vhost_redirect` 설정이 동작하지 않는 문제 수정 ( ``v2.10.2`` 부터 )
